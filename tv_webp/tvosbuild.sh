...
readonly TARGETDIR="${TOPDIR}/WebPTV.framework"
readonly DECTARGETDIR="${TOPDIR}/WebPDecoderTV.framework"
...
for PLATFORM in ${PLATFORMS}; do
  ARCH2=""
  MFLG="-miphoneos-version-min=6.0"
  if [[ "${PLATFORM}" == "AppleTVOS" ]]; then
    PLATFORM="AppleTVOS"
    ARCH="aarch64"
    ARCH2="arm64"
    MFLG="-mtvos-version-min=9.0"
...
${LIPO} -create ${LIBLIST} -output ${TARGETDIR}/WebPTV

cp -a ${SRCDIR}/src/webp/{decode,types}.h ${DECTARGETDIR}/Headers/
${LIPO} -create ${DECLIBLIST} -output ${DECTARGETDIR}/WebPDecoderTV
